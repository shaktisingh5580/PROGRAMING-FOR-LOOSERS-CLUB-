...
onAuthStateChanged(auth, user => {
    if (user) {
        ...
        notificationSettingsBtnEl.classList.remove('hidden'); // Show notification settings button

        if (assignmentAuthModalEl.classList.contains('active')) {
            assignmentAuthModalEl.classList.remove('active');
            document.body.style.overflow = '';
        }
        // Check initial notification permission status for the UI
        ...
        notificationSettingsBtnEl.classList.add('hidden'); // Hide notification settings button
...
async function saveNotificationSettings(event) {
    ...
    // If "all" is selected, it takes precedence
    ...
    if (Notification.permission === 'granted') {
        try {
            // IMPORTANT: Replace 'YOUR_PUBLIC_VAPID_KEY_HERE'
            const currentToken = await getToken(messaging, { vapidKey: 'BCZbrYYJ_xUYnUyq0fRtJx-VWmtMDUirXrrzqnX-nRvcDnIFlYkCEcvElKFP2_PkZAmO6j5ExR8KRJZu6Aj7X_s' });
            if (currentToken) {
        ...
        // If permission is not granted, we might want to remove an old token or just not include it
        // For simplicity, if permission isn't granted, we don't try to get/save a token.
        // If a token was previously stored, it remains unless explicitly cleared.
        // To clear a token if permission is revoked: prefData.fcmToken = deleteField(); or handle server-side.
    }
...
onMessage(messaging, (payload) => {
    ...
    const notificationOptions = {
        ...
        icon: payload.notification.icon || '/HummingBird (1).png' // Make sure this path is correct
    };
    // Display a browser notification (even in foreground)
    new Notification(notificationTitle, notificationOptions);
    // You could also show a custom in-app banner
    // alert(`New Assignment: ${payload.notification.title}\n${payload.notification.body}`);
});
...
document.addEventListener('DOMContentLoaded', () => {
    fetchAssignments();
    // Service worker registration for FCM (optional, but good practice to ensure it's registered)
    // The main token retrieval and onMessage handling is done above.
    // The firebase-messaging-sw.js will handle background messages.
    if ('serviceWorker' in navigator) {
        navigator.serviceWorker.register('/firebase-messaging-sw.js') // Ensure this path is correct
            .then((registration) => {
